  ...
  return (
    <div className="px-5 md:px-12 max-w-3xl mx-auto py-12 md:py-20">
      ...
      <p className="text-base md:text-lg text-ink-soft mb-10 max-w-xl">
        Thanks, {order.customer.name.split(" ")[0]}. We'll have everything packed and ready for you on{" "}
        <strong className="text-ink">{day?.label}</strong> at <strong className="text-ink">{slot?.label}</strong>.
      </p>

      <div className="bg-paper border border-rule p-5 md:p-6 mb-6">
        <p className="eyebrow mb-4">Pickup details</p>
        <p className="text-sm leading-relaxed">
          <strong>Where:</strong> 1244 Harbor View Dr, Apt 4B
          <br />
          San Francisco, CA
          <br />
        ...
        <p className="text-sm leading-relaxed mt-3">
          ...
          {order.payment === "cash"
            ? "Cash — due at pickup. Exact change appreciated."
            : order.payment === "applepay"
              ? "Apple Pay — paid."
              : "Card — paid."}
        </p>
      </div>
      ...
      <div className="bg-paper border border-rule p-5 md:p-6 mb-6 flex gap-4">
        ...
        <div>
          ...
          <p className="text-sm text-ink-soft">
            The day before pickup, plus a heads-up 2 hours before your slot. Sent to {order.customer.phone}.
          </p>
        </div>
      </div>
      ...
      {!account && !accepted && (
        <div className="bg-ink text-bg p-6 md:p-8">
          ...
          <p className="text-sm text-bg/80 mb-5">
            Create an account to reorder in one tap. We'll keep your contact details on file — nothing more.
          </p>
          <div className="flex gap-3">
            <button
              onClick={() => {
                setAccount({ name: order.customer.name, email: order.customer.email, phone: order.customer.phone });
                setAccepted(true);
              }}
            ...
            <button
              ...
              className="text-xs uppercase tracking-[0.2em] font-bold text-bg/60 hover:text-bg"
            >
              No thanks
            </button>
          </div>
      ...
      <div className="mt-10">
        <Link to="/" className="text-brand underline text-sm">
          ← Back to home
        </Link>
      </div>
    </div>
