...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Hand-rolled pasta and slow-fermented bread from a home kitchen. Pre-order this week's batch for Saturday pickup in San Francisco." },
    ...
    <div className="pb-24 md:pb-8">
        ...
        <div className="w-full md:w-1/2 px-5 md:px-12 py-8 md:py-12 flex flex-col justify-between border-b md:border-b-0 md:border-r border-rule animate-[slide-up_0.7s_var(--ease-out-expo)]">
          <div className="flex justify-between items-start">
            ...
            <div className="text-right">
              ...
              <p className="text-sm text-brand mt-1">{sat?.label} · {sat?.slots[0].label.split(" — ")[0]}–{sat?.slots[sat.slots.length - 1].label.split(" — ")[1]}</p>
            </div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <Link to="/menu" className="bg-brand text-brand-ink px-7 py-4 font-bold uppercase text-xs tracking-[0.2em] hover:brightness-110 transition-all text-center">
              Pre-Order for {sat?.label.split(",")[0]}
            </Link>
            <Link to="/contact" className="border border-ink/20 px-7 py-4 font-bold uppercase text-xs tracking-[0.2em] hover:bg-ink/5 transition-all text-center">
              Contact
            </Link>
          </div>
        </div>
      ...
      <section className="px-5 md:px-12 max-w-7xl mx-auto pt-16 md:pt-24">
              ...
              <div className="flex justify-between items-end mb-2 gap-4">
                <h3 className="font-display text-lg md:text-xl uppercase tracking-tight leading-tight">
                  ...
                  {item.popular && !item.soldOut && (
                    <span className="ml-2 text-[9px] font-bold tracking-[0.2em] text-brand align-middle">★ POPULAR</span>
                  )}
                </h3>
        ...
        <div className="mt-12 flex justify-center">
          <Link to="/menu" className="bg-ink text-bg px-8 py-4 font-bold uppercase text-xs tracking-[0.2em] hover:bg-brand transition-colors">
            View Full Menu →
          </Link>
        </div>
      </section>
      ...
      <section className="bg-ink text-bg px-5 md:px-12 py-20 mt-8">
        <div className="max-w-4xl mx-auto">
          ...
          <p className="text-lg md:text-xl text-bg/80 max-w-xl mb-8">
            Orders close <span className="text-brand-ink underline decoration-brand decoration-2 underline-offset-4">Thursday at 8pm</span>. Pickup is at {business.address}, {business.neighborhood}.
          </p>
          <Link to="/menu" className="bg-brand text-brand-ink inline-block px-8 py-4 font-bold uppercase text-xs tracking-[0.2em]">
            Start Pre-Order →
          </Link>
        </div>
      </section>
