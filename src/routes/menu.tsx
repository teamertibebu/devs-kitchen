...
export const Route = createFileRoute("/menu")({
  head: () => ({
    meta: [
      { title: "This Week's Menu — Dev's Kitchen" },
      { name: "description", content: "Browse this week's pre-order menu and reserve your Saturday pickup." },
      { property: "og:title", content: "This Week's Menu — Dev's Kitchen" },
    ...
    <div className="pb-32 md:pb-16">
      ...
      <section className="px-5 md:px-12 max-w-7xl mx-auto pt-10 md:pt-14 pb-8">
        ...
        <div className="mt-6 flex flex-wrap gap-x-8 gap-y-2 text-sm text-ink-soft">
          <span>
            <strong className="text-ink">Pickup:</strong> {sat?.label}
          </span>
          <span>
            <strong className="text-ink">Closes:</strong> Thu 8:00 PM
          </span>
          <span>
            <strong className="text-ink">Where:</strong> Harbor View Dr, SF
          </span>
        </div>
      </section>
                    ...
                    <article key={item.id} className={`group ${item.soldOut ? "opacity-60" : ""}`}>
                      ...
                      {!item.soldOut && (
                        ...
                        >
                          {inCart ? (
                            <>
                              <Check className="size-3" /> In bag ({inCart.qty})
                            </>
                          ) : (
                            <>
                              <Plus className="size-3" /> Add to bag
                            </>
                          )}
                        </button>
                      )}
      ...
      <section className="px-5 md:px-12 max-w-4xl mx-auto py-8">
        <Link
          to="/cart"
          className="hidden md:flex bg-ink text-bg px-8 py-4 font-bold uppercase text-xs tracking-[0.2em] hover:bg-brand transition-colors w-fit"
        >
          Review your bag →
        </Link>
      </section>
